...
import shared.geometry2d.x
import shared.geometry2d.y
...
data class Year2024Day13(
    ...
    fun partTwo() =
        machines.map { Machine(it.buttonA, it.buttonB, CONVERSION + it.prize.x() to CONVERSION + it.prize.y()) }
            .sumOf { it.solve() }
}
...
data class Machine(
    ...
    fun solve(): Long {
        val b =
            (buttonA.x() * prize.y() - buttonA.y() * prize.x()) / (buttonA.x() * buttonB.y() - buttonA.y() * buttonB.x())
    ...
    fun solveZ3() = Context().use { ctx ->
        ...
        solver.add(
            ctx.mkEq(
                ctx.mkInt(prize.first),
                ctx.mkAdd(ctx.mkMul(a, ctx.mkInt(buttonA.first)), ctx.mkMul(b, ctx.mkInt(buttonB.first)))
            )
        )
        solver.add(
            ctx.mkEq(
                ctx.mkInt(prize.y()),
                ctx.mkAdd(ctx.mkMul(a, ctx.mkInt(buttonA.y())), ctx.mkMul(b, ctx.mkInt(buttonB.y())))
            )
        )
