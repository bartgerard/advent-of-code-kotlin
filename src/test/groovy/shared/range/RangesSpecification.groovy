...
class RangesSpecification extends Specification {
    ...
    def "contains range"() {
        when:
        final containsRange = LongRangesKt.containsRange(range1, range2)
    ...
    def "is intersecting with"() {
        when:
        final isIntersecting = LongRangesKt.overlaps(range1, range2)
    ...
    def "used intersections"() {
        when:
        final usedIntersections = LongRangesKt.usedIntersections(ranges)
    ...
    def "gaps"() {
        when:
        final gaps = LongRangesKt.gaps(ranges)
    ...
    def "merge"() {
        when:
        final result = LongRangesKt.merge(ranges)
    ...
    def "merge one"() {
        when:
        final result = LongRangesKt.mergeOne(ranges as Collection, range)
    ...
    def "range without range"() {
        when:
        final remaining = LongRangesKt.without(range, subtrahend)
    ...
    def "range without ranges"() {
        when:
        final remaining = LongRangesKt.without(range, subtrahends)
    ...
    def "ranges without ranges"() {
        when:
        final remaining = LongRangesKt.without(ranges, subtrahends)
